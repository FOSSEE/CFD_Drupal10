...
/**
 * @file
 * Contains \Drupal\cfd_coursework_project\Form\CfdCourseworkProjectUploadAbstractCodeForm.
 */
...
class CfdCourseworkProjectUploadAbstractCodeForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'cfd_coursework_project_upload_abstract_code_form';
  }
  ...
  public function buildForm(array $form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    if ($proposal_q) {
      if ($proposal_data = $proposal_q->fetchObject()) {
        ...
        $form_state->setRedirect('cfd_coursework_project.upload_abstract_code_form');
        return [];
      }
    } //$proposal_q
    else {
      \Drupal::messenger()->addError(t('Invalid proposal selected. Please try again.'));
      $form_state->setRedirect('cfd_coursework_project.abstract');
      return [];
    }
    ...
    if ($abstracts_q) {
      if ($abstracts_q->is_submitted == 1) {
        \Drupal::messenger()->addError(t('You have already submitted your Coursework Project Directory, so you cannot upload another one. For any query, please write to us.'));
        $form_state->setRedirect('cfd_coursework_project.abstract');
        return [];
    ...
    $form['upload_coursework_project_developed_process'] = array(
            '#type' => 'file',
            '#title' => t('Upload the Coursework Project Directory'),
            //'#required' => TRUE,
            '#description' => t('<span style="color:red;">Current File :</span> ' . $existing_uploaded_S_file->filename . '<br />Separate filenames with underscore. No spaces or any special characters allowed in filename.') . '<br />' . t('<span style="color:red;">Allowed file extensions : ')
             . \Drupal::config('cfd_coursework_project.settings')->get('coursework_project_upload_extensions') 
            . '</span>',
        );
    ...
    // $form['cancel'] = array(
    //         '#type' => 'item',
    //         '#markup' => l(t('Cancel'), 'coursework-project/abstract-code'),
    //     );
$form['cancel'] = [
  '#type' => 'item',
  '#markup' => Link::fromTextAndUrl(
    t('Cancel'),
    Url::fromUserInput('/coursework-project/abstract-code')
  )->toString(),
];
    return $form;
  }
  ...
  public function validateForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    $upload = $files['upload_coursework_project_developed_process'] ?? NULL;
    if (!$upload || !$upload->isValid()) {
      $form_state->setErrorByName('upload_coursework_project_developed_process', $this->t('Please upload the abstract file'));
      return;
    }

    $allowed_extensions_str = \Drupal::config('cfd_coursework_project.settings')->get('coursework_project_upload_extensions');
    $allowed_extensions = array_filter(array_map('trim', explode(',', (string) $allowed_extensions_str)));
    if (empty($allowed_extensions)) {
      $form_state->setErrorByName('upload_coursework_project_developed_process', $this->t('Uploads are disabled until an administrator configures the allowed Coursework Project file extensions.'));
      return;
    }
    ...
    if (!in_array($temp_extension, $allowed_extensions, TRUE)) {
      $form_state->setErrorByName('upload_coursework_project_developed_process', $this->t('Only file with @ext extensions can be uploaded.', ['@ext' => $allowed_extensions_str]));
    }
    if ($upload->getSize() <= 0) {
      $form_state->setErrorByName('upload_coursework_project_developed_process', $this->t('File size cannot be zero.'));
    }
    if (!cfd_coursework_project_check_valid_filename($original_name)) {
      $form_state->setErrorByName('upload_coursework_project_developed_process', $this->t('Invalid file name specified. Only alphabets and numbers are allowed as a valid filename.'));
    }
  }
  ...
  public function submitForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    $proposal_id = $proposal_data->id;
    $query_s = "SELECT * FROM {coursework_project_submitted_abstracts} WHERE proposal_id = :proposal_id";
    $args_s = [":proposal_id" => $proposal_id];
    $query_s_result = \Drupal::database()->query($query_s, $args_s)->fetchObject();
    if (!$query_s_result) {
      $submitted_abstract_id = \Drupal::database()->insert('coursework_project_submitted_abstracts')
        ->fields([
        ...
        ->execute();
      \Drupal::database()->update('coursework_project_proposal')
      ...
      \Drupal::database()->update('coursework_project_submitted_abstracts')
        ->fields([
          'abstract_upload_date' => time(),
          'is_submitted' => 1,
        ])
    ...
    foreach ($files as $file_form_name => $upload) {
      ...
      if (!strstr($file_form_name, 'upload_coursework_project_developed_process')) {
        continue;
      }
      $file_type = 'S';
      ...
      $query_ab_f = "SELECT * FROM {coursework_project_submitted_abstracts_file} WHERE proposal_id = :proposal_id AND filetype = :filetype";
      $args_ab_f = [
        ":proposal_id" => $proposal_id,
        ":filetype" => $file_type,
      ];
      ...
      if (!$query_ab_f_result) {
        \Drupal::database()->insert('coursework_project_submitted_abstracts_file')
          ->fields([
        ...
        \Drupal::database()->update('coursework_project_submitted_abstracts_file')
          ->fields([
            'filename' => $original_name,
            'filepath' => $original_name,
            'filemime' => $filemime,
    ...
    // // This looks like another module's variable. You'll need to rewrite this call
    // // to ensure that it uses the correct configuration object.
    // $from = variable_get('coursework_project_from_email', '');
    ...
    if ($email_to) {
      $result = \Drupal::service('plugin.manager.mail')->mail('cfd_coursework_project', 'abstract_uploaded', $email_to, $langcode, $params, $from, TRUE);
  ...
  {
         $query = Database::getConnection()->select('coursework_project_submitted_abstracts_file');
