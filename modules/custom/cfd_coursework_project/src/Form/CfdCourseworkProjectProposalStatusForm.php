 ...
 */
class CfdCourseworkProjectProposalStatusForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'cfd_coursework_project_proposal_status_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $version_data = \Drupal::database()
      ->select('coursework_project_software_version')
      ->fields('coursework_project_software_version')
    ...
    $simulation_type_data = \Drupal::database()
      ->select('coursework_project_simulation_type')
      ->fields('coursework_project_simulation_type')
...
if ((int) $proposal_data->approval_status === 0) {
  $form['approve'] = [
    ...
    '#markup' => Link::fromTextAndUrl(
      $this->t('Click here'),
      Url::fromRoute('cfd_coursework_project.proposal_approval_form', ['id' => $proposal_id])
    )->toString(),
  ];
}
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValue('completed')) {
      $updated = \Drupal::database()->update('coursework_project_proposal')
        ->fields([
          'approval_status' => 3,
      ...
      cfd_coursework_project_create_readme_file($proposal_id);
      ...
      if ($email_to) {
        ...
        $params = [];
        $params['coursework_project_proposal_completed']['proposal_id'] = $proposal_id;
        $params['coursework_project_proposal_completed']['user_id'] = $proposal_data->uid;
        $params['coursework_project_proposal_completed']['headers'] = $headers;

        $langcode = $account ? $account->getPreferredLangcode() : \Drupal::languageManager()->getDefaultLanguage()->getId();
        $result = \Drupal::service('plugin.manager.mail')->mail('cfd_coursework_project', 'coursework_project_proposal_completed', $email_to, $langcode, $params, $from, TRUE);
      ...
      $this->messenger()->addStatus($this->t('Congratulations! CFD Coursework Project proposal has been marked as completed. User has been notified of the completion.'));
