...
/**
 * @file
 * Contains \Drupal\cfd_coursework_project\Form\CfdCourseworkProjectProposalForm.
 */
...
class CfdCourseworkProjectProposalForm extends FormBase {
  ...
  public function getFormId() {
    return 'cfd_coursework_project_proposal_form';
  }
  ...
  public function buildForm(array $form, \Drupal\Core\Form\FormStateInterface $form_state, $no_js_use = NULL) {
    ...
    if ($user->id() == 0) {
      $msg = \Drupal::messenger()->addError(t('It is mandatory to @login_link on this website to access the coursework project proposal form. If you are a new user, please create a new account first.', [
...
// ->send();
    ...
    $form['faculty_name'] = [
      '#type' => 'textfield',
      '#title' => t('Name of the Faculty Member of your Institution, if any, who helped you with this Coursework Project'),
      // '#size' => 50,
      '#maxlength' => 50,
      '#validated' => TRUE,
      '#description' => t('<span style="color:red">Maximum character limit is 50</span>'),
    ...
    $form['faculty_department'] = [
      '#type' => 'textfield',
      '#title' => t('Department of the Faculty Member of your Institution, if any, who helped you with this Coursework Project'),
      // '#size' => 50,
      '#maxlength' => 50,
      '#validated' => TRUE,
      '#description' => t('<span style="color:red">Maximum character limit is 50</span>'),
    ...
    $form['faculty_email'] = [
      '#type' => 'textfield',
      '#title' => t('Email id of the Faculty Member of your Institution, if any, who helped you with this Coursework Project'),
      // '#size' => 255,
      '#maxlength' => 255,
      '#validated' => TRUE,
    ...
    $list_coursework_project = cfd_coursework_project_list();
    if (!empty($list_coursework_project)) {
      $form['cfd_project_title_check'] = [
        '#type' => 'radios',
        '#title' => t('Is the proposed CFD Coursework project from the list of available CFD Coursework projects?'),
        '#options' => [
          '1' => 'Yes',
          '0' => 'No',
        ],
      ...
      $form['cfd_coursework_project_name_dropdown'] = [
        '#type' => 'select',
        '#title' => t('Select an available Coursework Project'),
        '#required' => FALSE,
        '#options' => cfd_coursework_project_list(),
        '#validated' => TRUE,
        '#states' => [
          'visible' => [
    ...
    if ($simulation_id < self::MODIFIED_SIMULATION_TYPE_ID) {
      $solver_options = cfd_coursework_project_list_solvers($simulation_id);
    ...
    $form['abstract_file']['abstract_file_path'] = [
      '#type' => 'file',
      // '#size' => 48,
      '#description' => t('<span style="color:red;">Upload filenames with allowed extensions only. No spaces or any special characters allowed in filename.</span>') . '<br />' . t('<span style="color:red;">Allowed file extensions: ') . \Drupal::config('cfd_coursework_project.settings')->get('resource_upload_extensions') . '</span>',
  ];
  // var_dump(\Drupal::config('cfd_coursework_project.settings')->get('default_allowed_extensions'));die;
    ...
    $form['term_condition'] = [
      ...
      '#description' => t('<a href="/coursework-project/term-and-conditions" target="_blank">View Terms and Conditions</a>'),
      '#required' => TRUE,
    ];
    $form['submit'] = [
      '#type' => 'submit',
  ...
  public function validateForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    if ($form_state->getValue('cfd_project_title_check') == 1) {
      $project_title = trim((string) $form_state->getValue('cfd_coursework_project_name_dropdown'));
      if ($project_title === '') {
        $form_state->setErrorByName('cfd_coursework_project_name_dropdown', t('Please select an available Coursework Project.'));
      }
    }
    else {
      $project_title = trim((string) $form_state->getValue('project_title'));
      if ($project_title === '') {
        $form_state->setErrorByName('project_title', t('Project title should not be empty.'));
      }
    }
    $form_state->setValue('project_title', $project_title);
    ...
    $form_state->setValue('project_title', trim((string) $form_state->getValue('project_title')));
    ...
    $allowed_extensions_str = \Drupal::config('cfd_coursework_project.settings')->get('resource_upload_extensions');
    $allowed_extensions = array_filter(array_map('trim', explode(',', (string) $allowed_extensions_str)));
    if (empty($allowed_extensions)) {
      $form_state->setErrorByName('abstract_file_path', t('Uploads are disabled until an administrator configures the allowed resource-file extensions.'));
      return;
    }
  ...
  public function submitForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    if ($form_state->getValue(['cfd_project_title_check']) == 1) {
      $project_title = $form_state->getValue(['cfd_coursework_project_name_dropdown']);
    }
    else {
    ...
    $directory_name = cfd_coursework_project_directory_name($project_title, $proposar_name);
    $file_system = \Drupal::service('file_system');
    $target_dir = $root_path . $directory_name . '/';
    if (!$file_system->prepareDirectory($target_dir, FileSystemInterface::CREATE_DIRECTORY | FileSystemInterface::MODIFY_PERMISSIONS)) {
      $this->messenger()->addError($this->t('The Coursework Project upload directory could not be created. Please contact the site administrator.'));
      return;
    }
    ...
    $proposal_id = $connection->insert('coursework_project_proposal')
      ->fields([
        'uid' => $user->id(),
        'approver_uid' => 0,
        'name_title' => $v['name_title'],
        'contributor_name' => cfd_coursework_project_sentence_case(trim($v['contributor_name'])),
        'contact_no' => $v['contributor_contact_no'],
        'university' => $v['university'],
        'institute' => cfd_coursework_project_sentence_case($v['institute']),
    ...
    if ($upload && $upload->isValid()) {
      ...
      if (file_exists($target_path)) {
        ...
        try {
          ...
          $submitted_abstract_id = $connection->insert('coursework_project_submitted_abstracts')
            ->fields([
          ...
          $connection->insert('coursework_project_submitted_abstracts_file')
            ->fields([
   ...
   // $email_to = $user->mail;
    ...
    // $form = variable_get('coursework_project_from_email', '');

    // @FIXME
    // // @FIXME
    // // This looks like another module's variable. You'll need to rewrite this call
    // // to ensure that it uses the correct configuration object.
    // $bcc = variable_get('coursework_project_emails', '');

    // @FIXME
    // // @FIXME
    // // This looks like another module's variable. You'll need to rewrite this call
    // // to ensure that it uses the correct configuration object.
    // $cc = variable_get('coursework_project_cc_emails', '');

    // $params['coursework_project_proposal_received']['proposal_id'] = $proposal_id;
    // $params['coursework_project_proposal_received']['user_id'] = $user->uid;
    // $params['coursework_project_proposal_received']['headers'] = [
    //   'From' => $form,
    //   'MIME-Version' => '1.0',
    ...
    //   'Bcc' => $bcc,
    // ];
    // if (!\Drupal::service('plugin.manager.mail')->mail('cfd_coursework_project', 'coursework_project_proposal_received', $email_to, 'en', $params, $form, TRUE)) {
...
$params['coursework_project_proposal_received']['proposal_id'] = $proposal_id;
$params['coursework_project_proposal_received']['user_id'] = $user->id();
$headers = [
  'From' => $from,
  'MIME-Version' => '1.0',
  'Content-Type' => 'text/plain; charset=UTF-8; format=flowed; delsp=yes',
...
$params['coursework_project_proposal_received']['headers'] = $headers;
...
if ($email_to) {
  $result = \Drupal::service('plugin.manager.mail')->mail('cfd_coursework_project', 'coursework_project_proposal_received', $email_to, $langcode, $params, $from, TRUE);
  if (empty($result['result'])) {
    ...
    \Drupal::messenger()->addStatus(t('We have received your coursework project proposal. We will get back to you soon.'));
