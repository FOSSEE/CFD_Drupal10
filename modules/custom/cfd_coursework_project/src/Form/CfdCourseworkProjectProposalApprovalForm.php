...
/**
 * @file
 * Contains \Drupal\cfd_coursework_project\Form\CfdCourseworkProjectProposalApprovalForm.
 */
...
class CfdCourseworkProjectProposalApprovalForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'cfd_coursework_project_proposal_approval_form';
  }
  ...
  public function buildForm(array $form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    $query_abstract = \Drupal::database()->select('coursework_project_submitted_abstracts_file');
    $query_abstract->fields('coursework_project_submitted_abstracts_file');
    ...
    if ($proposal_q) {
      if ($proposal_data = $proposal_q->fetchObject()) {
        ...
        // drupal_goto('coursework-project/manage-proposal');
        return;
      }
    } //$proposal_q
    else {
  ...
  $form['abstract_file_path'] = [
    ...
    '#markup' => Link::fromTextAndUrl(
      $resource_file,
      Url::fromRoute('cfd_coursework_project.project_files', ['proposal_id' => $proposal_id])
    )->toString(),
  ];
    ...
    $form['approval'] = [
      '#type' => 'radios',
      '#title' => t('CFD Coursework Project proposal'),
      '#options' => [
        '1' => 'Approve',
        '2' => 'Disapprove',
      ],
    ...
    // $form['cancel'] = array(
    //         '#type' => 'item',
    //         '#markup' => l(t('Cancel'), 'coursework-project/manage-proposal'),
    //     );
  ...
  public function submitForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    if ($proposal_q) {
      if ($proposal_data = $proposal_q->fetchObject()) {
        ...
        \Drupal::messenger()->addError(t('Invalid proposal selected. Please try again.'));
        $form_state->setRedirect('cfd_coursework_project.proposal_pending');
        return;
      }
    } //$proposal_q
    else {
    ...
    if ($form_state->getValue(['approval']) == 1) {
      $query = "UPDATE {coursework_project_proposal} SET approver_uid = :uid, approval_date = :date, approval_status = 1 WHERE id = :proposal_id";
      $args = [
        ":uid" => $user->id(),
        ":date" => time(),
      ...
      $params['coursework_project_proposal_approved']['proposal_id'] = $proposal_id;
      $params['coursework_project_proposal_approved']['user_id'] = $proposal_data->uid;
      $headers = [
        'From' => $from,
        'MIME-Version' => '1.0',
      ...
      $params['coursework_project_proposal_approved']['headers'] = $headers;

      $langcode = $user_data ? $user_data->getPreferredLangcode() : \Drupal::languageManager()->getDefaultLanguage()->getId();
      if ($email_to) {
        $result = \Drupal::service('plugin.manager.mail')->mail('cfd_coursework_project', 'coursework_project_proposal_approved', $email_to, $langcode, $params, $from, TRUE);
        if (empty($result['result'])) {
          \Drupal::messenger()->addError('Error sending email message.');
        }
      }

      \Drupal::messenger()->addStatus('CFD Coursework Project proposal No. ' . $proposal_id . ' approved. User has been notified of the approval.');
      ...
      if ($form_state->getValue(['approval']) == 2) {
        $query = "UPDATE {coursework_project_proposal} SET approver_uid = :uid, approval_date = :date, approval_status = 2, dissapproval_reason = :dissapproval_reason WHERE id = :proposal_id";
        $args = [
          ":uid" => $user->id(),
          ":date" => time(),
...
$email_to = $user_data ? $user_data->getEmail() : '';
        ...
        $bcc = $config->get('coursework_project_emails');
        $cc = $config->get('coursework_project_cc_emails');
        $params['coursework_project_proposal_disapproved']['proposal_id'] = $proposal_id;
        $params['coursework_project_proposal_disapproved']['user_id'] = $proposal_data->uid;
        $headers = [
          'From' => $from,
          'MIME-Version' => '1.0',
          'Content-Type' => 'text/plain; charset=UTF-8; format=flowed; delsp=yes',
        ...
        $params['coursework_project_proposal_disapproved']['headers'] = $headers;

        $langcode = $user_data ? $user_data->getPreferredLangcode() : \Drupal::languageManager()->getDefaultLanguage()->getId();
        if ($email_to) {
          $result = \Drupal::service('plugin.manager.mail')->mail('cfd_coursework_project', 'coursework_project_proposal_disapproved', $email_to, $langcode, $params, $from, TRUE);
          if (empty($result['result'])) {
            \Drupal::messenger()->addError('Error sending email message.');
          }
        }

        \Drupal::messenger()->addError('CFD Coursework Project proposal No. ' . $proposal_id . ' dis-approved. User has been notified of the dis-approval.');
        \Drupal\Core\Cache\Cache::invalidateTags([
          'coursework_project_proposal_list',
          "coursework_project_proposal:$proposal_id",
        ]);
