...
/**
 * @file
 * Contains \Drupal\cfd_coursework_project\Form\AddProjectTitleForm.
 */
...
class AddProjectTitleForm extends FormBase {
  ...
  public function getFormId() {
    return 'cfd_coursework_project_add_project_title_form';
  }
  ...
  public function buildForm(array $form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    if ($user->id() == 0) {
      $this->messenger()->addError($this->t('It is mandatory to @login_link on this website to access the coursework project proposal form. If you are a new user, please create a new account first.', [
    ...
    //  if ($user->uid == 0) {
    //   $msg = \Drupal::messenger()->addError(t('It is mandatory to ' . \Drupal\Core\Link::fromTextAndUrl('login', \Drupal\Core\Url::fromRoute('user.page')) . ' on this website to access the coursework project proposal form. If you are new user please create a new account first.'));
    //   drupal_goto('user');
    //   return $msg;
    // } //$user->uid == 0
    $form['#attributes'] = [
    ...
    $form['upload_project_title_resource_file']['project_title_resource_file_path'] = [
      '#type' => 'file',
      // '#size' => 48,
      '#description' => $this->t('<span style="color:red;">Upload filenames with allowed extensions only. No spaces or any special characters allowed in filename.</span>') . '<br />' . $this->t('<span style="color:red;">Allowed file extensions: ') . \Drupal::config('cfd_coursework_project.settings')->get('list_of_available_projects_file_extensions') . '</span>',
    ];
  ...
  public function validateForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    $allowed_extensions_str = \Drupal::config('cfd_coursework_project.settings')->get('list_of_available_projects_file_extensions');
    $allowed_extensions = array_filter(array_map('trim', explode(',', (string) $allowed_extensions_str)));
    if (empty($allowed_extensions)) {
      $form_state->setErrorByName('project_title_resource_file_path', $this->t('Uploads are disabled until an administrator configures the allowed project-title resource extensions.'));
      return;
    }
  ...
  public function submitForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    $result1 = \Drupal::database()->insert('coursework_project_titles')
      ->fields([
        'project_title_name' => $v['new_project_title_name'],
      ])
      ->execute();
    $dest_path = cfd_coursework_project_titles_resource_file_path();
    ...
    if ($upload && $upload->isValid()) {
      ...
      if (file_exists($target_path)) {
        ...
        \Drupal::database()->update('coursework_project_titles')
    ...
    \Drupal\Core\Cache\Cache::invalidateTags(['coursework_project_titles_list']);
  }
